...
unassign` (see `forgeops/cli/task.py`): read-only preflight-plan
builders (`build_task_assign_plan`/`build_task_unassign_plan`) and the
single mutating apply step for each
(`apply_task_assign`/`apply_task_unassign`), following the same
plan/apply split every other mutating ForgeOps command uses.

Ownership is one-to-one and stored only through the two existing
managed records this checkpoint's predecessors already reserved a slot
for - `TASK.json.worktree_id` (`forgeops/state/task_registry.py`) and
`WORKTREE_REGISTRY.json`'s per-record `task_id`
(`forgeops/state/worktree_registry.py`) - no secondary ownership
database. Both are treated as equally authoritative: a successful
assign/unassign writes both together, rolling back the first if the
second fails, so ownership is never split across the two records
(`TASK_INDEX.json`'s own summary copy remains bookkeeping only, exactly
as `task create`/`task close` already treat it - a failure updating it
after both authoritative records already agree is a warning, never a
failure)."""

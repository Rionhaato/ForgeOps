...
- **`task create`/`task show`/`task list`/`task validate`/`task close`/
  ...
  preflight conflict (worktree assignment's own eligibility conflicts
  include the task/worktree not existing, either already being
  assigned, a removed/locked/stale/protected worktree, or a terminal
  task; agent assignment's own mirror these for the task/agent pair -
  see `docs/tasks.md` "Ownership" and `docs/agents.md` "Ownership"),
  `--confirm` missing on a non-dry-run `task close`/`task unassign`/
  `task unassign-agent` (`task assign`/`task assign-agent` themselves
  need no `--confirm`, only `--dry-run`), `task show`/`task validate`
  given a task ID that cannot be located at all, or any blocking finding
  from `task validate` itself (including its worktree- and
  agent-ownership-consistency checks); `COMMAND_EXECUTION_FAILURE`
  (5) also if a `task create`/`task close`/`task assign`/`task
  unassign`/`task assign-agent`/`task unassign-agent` write genuinely
  fails partway through (already-created/already-updated paths rolled
  back where safe - see `docs/tasks.md`); otherwise `WARNINGS_PRESENT`
  (1) if `task create`/`task close`/`task assign`/`task unassign`/`task
  assign-agent`/`task unassign-agent` succeeded but its
  `TASK_INDEX.json` update failed, `task assign-agent` succeeded on a
  task with no assigned worktree (a non-blocking `task-has-no-worktree`
  warning), `task list` found a stale or unindexed entry (or a
  malformed index), or `task show`/`task validate` found only
  non-blocking issues; otherwise `SUCCESS` (0), including a
  conflict-free `--dry-run` for any mutating command. See
  `docs/tasks.md`.

- **`agent register`/`agent list`/`agent show`**: `REPO_NOT_FOUND` (4) /
  `COMMAND_EXECUTION_FAILURE` (5, git unavailable) as above; `BLOCKED`
  (2) for the read-only reference repository, an uninitialized project
  (the same shared gate `task` commands use), any `agent register`
  preflight conflict (invalid/duplicate/secret-shaped ID, invalid kind,
  oversized/secret-shaped display name, a malformed registry), or
  `agent show` given an ID that cannot be located at all;
  `COMMAND_EXECUTION_FAILURE` (5) also if `agent register`'s single
  atomic write genuinely fails; otherwise `WARNINGS_PRESENT` (1) if
